...
void InputServiceTest::SetUp() {
    ...
    inputService = std::make_unique<services::InputService>(
        *mockGpioInput, *mockEventQueue, *mockQueue, *mockTaskRunner, *mockClock);
}
...
TEST_F(InputServiceTest, tc09_init_deinit_success) {
    EXPECT_CALL(*mockTaskRunner, start(_, _, _, _))
        .WillOnce([&](const common::TaskParams&, uint32_t, common::StepFn fn, void* user) {
            stepFn = fn;
            stepUser = user;
